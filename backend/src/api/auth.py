...
class RegisterInput(BaseModel):
    ...
    # Student fields (only filled if role is student)
    cgpa: float = None
    major: str = None
    graduation_year: int = None

    # Recruiter fields (only filled if role is recruiter)
    company_id: int = None
...
def register(data: RegisterInput):
    ...
    try:
        ...
        # Insert into Students or Recruiters depending on role
...
def login(data: LoginInput):
    ...
    try:
        ...
        # Create JWT token containing account_id, name and role

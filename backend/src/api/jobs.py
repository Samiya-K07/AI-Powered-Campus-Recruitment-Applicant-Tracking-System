...
class SkillRequirement(BaseModel):
    skill_id: int
    weight: float  # 1.0 = required, 0.5 = preferred
...
# Get All Companies: for recruiter registration dropdown 
...
# Create a New Company: for recruiter registration 
...
# Reopen a Job Posting: Recruiter Only 
...
def delete_job(
    ...
    try:
        ...
        if app_count > 0:
            # Applications exist => close instead of delete to protect history
            cur.execute(
                "UPDATE Job_Posts SET status = 'closed' WHERE job_id = %s",
                (job_id,)
            )
        ...
        # Must delete from Job_Required_Skills first due to foreign key
        cur.execute(
...
# Get Audit Log for a Job's Applications: Recruiter Only 
...
def get_job_audit_log(
    ...
    try:
        ...
        # Fetch all audit log entries for this job's applications
        cur.execute(
            """
            SELECT
                al.log_id,
...
def post_job(
    ...
    try:
        ...
        # Check for duplicate active job posting
        cur.execute(
            """

...
# Agent 1 Helper: Extract Experience Years 
def extract_experience_years(text: str) -> int:
    ...
    # Pattern: Month Year - Month Year only
    pattern = (
...
# Submit Resume: Agent 1 Runs Here
@router.post("/resume")
def submit_resume(
    data: ResumeInput,
    current_user: dict = Depends(get_current_student)
    ...
    try:
        ...
        # Step 1: Load all known skills from Skills table
        cur.execute("SELECT skill_id, skill_name FROM Skills")
        all_skills = cur.fetchall()

        # Step 2: Scan resume text for matching skill names (case-insensitive)
        found_skill_ids   = []
        found_skill_names = []
        ...
        # Step 3: Extract years of experience using month-year pattern only
        experience_years = extract_experience_years(data.raw_text)

        # Step 4: Save or update resume in Resumes table
        cur.execute(
        ...
        # Step 5: Add found skills to Student_Skills
        # Skip any skill the student already has to avoid duplicates
...
# Get All Skills: for dropdown on frontend 
